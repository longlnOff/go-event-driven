...
func (v VipBundleRepository) Add(ctx context.Context, vipBundle ticketsEntity.VipBundle) error {
	...
	return updateInTx(
		...
		func(ctx context.Context, tx *sqlx.Tx) error {
			...
			err = ticketsEvent.NewEventBus(
				outboxPublisher,
				watermill.NewSlogLogger(log.FromContext(ctx)),
			).Publish(
				ctx, ticketsEntity.VipBundleInitialized_v1{
...
func (v VipBundleRepository) Get(
	ctx context.Context,
...
func (v VipBundleRepository) vipBundleByID(
	ctx context.Context,
	vipBundleID ticketsEntity.VipBundleID,
	db Executor,
) (ticketsEntity.VipBundle, error) {
	...
	if err != nil {
		return ticketsEntity.VipBundle{}, fmt.Errorf("could not get vip bundle: %w", err)
	}

	var vipBundle ticketsEntity.VipBundle
	err = json.Unmarshal(payload, &vipBundle)
	if err != nil {
		return ticketsEntity.VipBundle{}, fmt.Errorf("could not unmarshal vip bundle: %w", err)
	}

	return vipBundle, nil
}

func (v VipBundleRepository) GetByBookingID(ctx context.Context, bookingID uuid.UUID) (ticketsEntity.VipBundle, error) {
	return v.getByBookingID(ctx, bookingID, v.db)
}

func (v VipBundleRepository) getByBookingID(
	ctx context.Context,
	bookingID uuid.UUID,
	db Executor,
) (ticketsEntity.VipBundle, error) {
...
func (v VipBundleRepository) UpdateByID(
	...
	var vb ticketsEntity.VipBundle

	err := updateInTx(
		ctx, v.db, sql.LevelSerializable, func(ctx context.Context, tx *sqlx.Tx) error {
			var err error
	...
	if err != nil {
		return ticketsEntity.VipBundle{}, fmt.Errorf("could not update vip bundle: %w", err)
	}
